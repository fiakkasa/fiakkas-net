...
) : IEmailService
{
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Failed to send email from {SenderAddress} to {RecipientAddress}",
                senderAddress,
                recipientAddress
            );
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Failed to send email from {SenderAddress} to {RecipientAddress}",
                nameof(EmailConfig.DefaultSenderAddress),
                recipientAddress
            );
    ...
    )
    {
        ...
        {
            From = senderAddress.GetSenderMailAddress(
                emailConfig.AlwaysUseDefaultSenderAddress,
